...
run_test = int(config.get("type","test"))
morning_go_hour = int(config.get("time","morning_go_hour"))
morning_back_hour = int(config.get("time","morning_back_hour"))
afernoon_go_hour = int(config.get("time","afernoon_go_hour"))
afternoon_back_hour = int(config.get("time","afternoon_back_hour"))
...
def with_open_close_dingding(func):
    def wrapper(self, *args, **kwargs):
        ...
        print("打开打卡界面")
        operation_list1 = [self.adbselect_work, self.adbselect_shouqi,self.adbselect_playcard]
...
class dingding:
    ...
    def __init__(self,directory):
        ...
        # 点击收起
        self.adbselect_shouqi = '"%s\\adb" shell input tap %s' % (directory,config.get("position","shouqi"))
        # 点击考勤打卡
    ...
    def openplaycard_interface(self):
        ...
        operation_list = [self.adbselect_work]
        for operation in operation_list:
            process = subprocess.Popen(operation, shell=False,stdout=subprocess.PIPE)
            process.wait()
            time.sleep(2)
...
def incode_loop(func,minute):
    ...
    # 判断时间当超过上班时间则打下班卡。否则则打上班卡。
    if datetime.datetime.now().hour <=morning_go_hour:
        # 用来分类上班和下班。作为参数传入任务调度
        hourtype = 1
        print("下次将在", str(morning_go_hour), ":", str(minute), "打卡")
    elif datetime.datetime.now().hour ==morning_back_hour:
        hourtype = 2
        print("下次将在", str(morning_back_hour), ":", str(minute), "打卡")
    elif datetime.datetime.now().hour <= afernoon_go_hour:
        hourtype = 3
        print("下次将在", str(afernoon_go_hour), ":", str(minute), "打卡")
    elif datetime.datetime.now().hour >= afternoon_back_hour:
        hourtype = 4
        print("下次将在", str(afternoon_back_hour), ":", str(minute), "打卡")
    #执行任务调度函数
...
def start_loop(hourtype,minute):
    ...
    if hourtype == 1 and now_hour == morning_go_hour and now_minute == minute and is_weekend():
        random_time = random_minute()
        dingding(directory).goto_work(random_time)
        scheduler.enter(0,0,incode_loop,(start_loop,random_time,))
    if hourtype == 2 and now_hour == morning_back_hour and now_minute == minute and is_weekend():
        random_time = random_minute()
        dingding(directory).after_work(random_time)
        scheduler.enter(0, 0, incode_loop,(start_loop,random_time,))
    if hourtype == 3 and now_hour == afernoon_go_hour and now_minute == minute and is_weekend():
        random_time = random_minute()
        dingding(directory).goto_work(random_time)
        scheduler.enter(0,0,incode_loop,(start_loop,random_time,))
    if hourtype == 4 and now_hour == afternoon_back_hour and now_minute == minute and is_weekend():
        random_time = random_minute()
        dingding(directory).after_work(random_time)
        scheduler.enter(0, 0, incode_loop,(start_loop,random_time,))
    else:
